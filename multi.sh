...
for x in u8_*.c; do NAME=`basename $x .c | sed s/u8_/i32_/g`; cp $x $NAME.c; sed -i s/u8/i32/g $NAME.c; sed -i s/uint8_t/int32_t/g $NAME.c; done
for x in u8_*.c; do NAME=`basename $x .c | sed s/u8_/i64_/g`; cp $x $NAME.c; sed -i s/u8/i64/g $NAME.c; sed -i s/uint8_t/int64_t/g $NAME.c; done
